...
/* Include the made file with my code: Koen - Line:137 recieve the message*/
...
/* Main routine of the process manager. */
  ...
  /* MY WORK: Koen*/
  struct rt_struct realtime_info;
  /* a struct to keep the rt_data retrieved from the message */
  /* END O STUFF */
  /* SEF local startup. */
  sef_local_startup();
  ...
  while (TRUE) {
	...
	case PM_SETGROUPS_REPLY:
		if (who_e == VFS_PROC_NR)
		{
			...
			result= ENOSYS;
		break;
	case VFS_SAYS_REALTIME:
		realtime_info = handle_vfs_message(m_in);
	default:
