...
_PROTOTYPE( int do_setrt, (struct rt_k_struct) ); 
/* prototype for set_rt, with an if statment which disables it when a var in config.h is changed */
#if ! USE_SETRT
#define do_setrt NULL
#endif

_PROTOTYPE( int do_wfnp, (void) );
/* see do_setrt */
# if ! USE_WFNP
# define do_wfnp NULL
# endif
